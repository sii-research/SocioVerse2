"""End-to-end + parity for abm_civil_violence (umbrella).

The hardest parity case: agents disappear (jailed) AND the env's apply draws heavily on
its RNG (cop arrests, jail terms, releases, movement). The citizen decision rule is
deterministic, so the action set — and hence the apply-RNG sequence — is identical
between native and SocioVerse2 (same seed, agent-id-ordered), giving exact parity.
"""
...
import studies.abm_civil_violence as study  # noqa: F401  (registers abm.* providers)
...
def _run_sv(n_steps: int, seed: int, tmp_path):
    ...
    sim = build_simulator(env_bundle=env_b, pop_bundle=pop_b, sim_config=sim_c,
                          store_path=tmp_path / "abm_civil_violence.duckdb")
    return s, sim, sim.run()


def test_runs_with_bursts_and_arrests(tmp_path):
    s, _sim, hist = _run_sv(30, 42, tmp_path)
    assert len(hist.rows) == 31
    assert hist.covers(s.metrics) == []
    assert max(r["active"] for r in hist.rows) > 0          # rebellion breaks out
    assert hist.rows[-1]["jailed"] > 0                      # arrests happen


def test_persistent_ids(tmp_path):
    _, sim, _ = _run_sv(3, 42, tmp_path)
    personas = sim.population.build(seed=42)
    assert personas[0].agent_id == "civil_violence-0000"
    assert len({p.agent_id for p in personas}) == len(personas)


def test_parity_with_native(tmp_path):
    _, _, hist = _run_sv(30, 42, tmp_path)
    native = _native_trajectory(30, 42)
