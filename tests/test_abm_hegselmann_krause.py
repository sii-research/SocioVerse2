"""End-to-end + parity for abm_hegselmann_krause (umbrella).

HK's context carries a NumPy peers array (proving the umbrella handles numpy-valued
quadrants), and its rule is deterministic, so parity is exact.
"""
...
import studies.abm_hegselmann_krause as study  # noqa: F401  (registers abm.* providers)
...
TASK = "hegselmann_krause"
KEYS = ("num_clusters", "spread", "variance")
...
def _run_sv(n_steps: int, seed: int, tmp_path):
    ...
    sim = build_simulator(env_bundle=env_b, pop_bundle=pop_b, sim_config=sim_c,
                          store_path=tmp_path / "abm_hk.duckdb")
    return s, sim, sim.run()


def test_runs_and_coalesces(tmp_path):
    s, _sim, hist = _run_sv(10, 42, tmp_path)
    assert len(hist.rows) == 11
    assert hist.covers(s.metrics) == []
    assert hist.rows[-1]["num_clusters"] < hist.rows[0]["num_clusters"]   # opinions coalesce


def test_persistent_ids(tmp_path):
    _, sim, _ = _run_sv(3, 42, tmp_path)
    personas = sim.population.build(seed=42)
    assert personas[0].agent_id == "hegselmann_krause-0000"
    assert len({p.agent_id for p in personas}) == len(personas)


def test_parity_with_native(tmp_path):
    _, _, hist = _run_sv(10, 42, tmp_path)
    native = _native_trajectory(10, 42)
