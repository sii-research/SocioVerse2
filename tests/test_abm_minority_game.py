"""End-to-end + parity for abm_minority_game (umbrella).

Context carries NumPy strategy tables + virtual scores. The random tie-break uses the
...
import studies.abm_minority_game as study  # noqa: F401  (registers abm.* providers)
...
def _run_sv(n_steps: int, seed: int, tmp_path):
    ...
    sim = build_simulator(env_bundle=env_b, pop_bundle=pop_b, sim_config=sim_c,
                          store_path=tmp_path / "abm_minority.duckdb")
...
def test_parity_with_native(tmp_path):
    ...
    for t, (sv_row, nat) in enumerate(zip(hist.rows, native)):
        assert sv_row["volatility"] == pytest.approx(nat["volatility"]), f"step {t} volatility drift"
        if t >= 1:                                          # attendance is None at step 0
            assert sv_row["attendance"] == nat["attendance"], f"step {t} attendance drift"
