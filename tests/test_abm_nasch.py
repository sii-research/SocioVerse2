"""End-to-end + parity for abm_nasch (umbrella).

NaSch's rule has a per-vehicle random slowdown (a module-level RNG), so the parity
reference must consume that RNG in the SAME order the runtime does — it decides in
persistent-agent-id order, so the native reference sorts observations by agent_id too.
With that alignment the wrapped trajectory reproduces the native one exactly.
"""
...
import studies.abm_nasch as study  # noqa: F401  (registers abm.* providers via umbrella)
...
def _native_trajectory(n_steps: int, seed: int) -> list[dict]:
    ...
    for _t in range(1, n_steps + 1):
        obs = sorted(env.observe_batch(), key=lambda o: o.agent_id)   # match the runtime's id order
...
def _run_sv(n_steps: int, seed: int, tmp_path):
    ...
    sim = build_simulator(env_bundle=env_b, pop_bundle=pop_b, sim_config=sim_c,
                          store_path=tmp_path / "abm_nasch.duckdb")
...
def test_runs(tmp_path):
    ...
    vmax = 5
    assert hist.rows[-1]["n"] == 50
    assert 0.0 <= hist.rows[-1]["mean_speed"] <= vmax
    assert hist.rows[-1]["flow"] >= 0.0


def test_persistent_ids(tmp_path):
    _, sim, _ = _run_sv(3, 42, tmp_path)
    personas = sim.population.build(seed=42)
    assert personas[0].agent_id == "nasch-0000"
