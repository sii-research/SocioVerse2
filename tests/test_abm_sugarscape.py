"""End-to-end + parity for abm_sugarscape (umbrella).

First disappearing-agent task: citizens starve and leave the grid. The runtime's persona pool
stays fixed; the umbrella returns an empty Observation for departed agents and the
decision skips them. Parity is exact (verified) because the active set and action order
are identical (agent-id order + same seed).
"""
...
import studies.abm_sugarscape as study  # noqa: F401  (registers abm.* providers)
...
def _run_sv(n_steps: int, seed: int, tmp_path):
    ...
    sim = build_simulator(env_bundle=env_b, pop_bundle=pop_b, sim_config=sim_c,
                          store_path=tmp_path / "abm_sugarscape.duckdb")
    return s, sim, sim.run()


def test_runs_with_starvation_and_inequality(tmp_path):
    s, _sim, hist = _run_sv(30, 42, tmp_path)
    assert len(hist.rows) == 31
    assert hist.covers(s.metrics) == []
    assert hist.rows[-1]["alive"] < hist.rows[0]["alive"]   # some citizens starve
    assert 0.0 <= hist.rows[-1]["gini"] <= 1.0


def test_persistent_ids(tmp_path):
    _, sim, _ = _run_sv(3, 42, tmp_path)
    personas = sim.population.build(seed=42)
    assert personas[0].agent_id == "sugarscape-0000"
